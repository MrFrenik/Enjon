...
namespace BeastBehaviorTree
{
	...
	inline BT::BehaviorTree* BeastTree()
	{
		...
		BT::SimpleTask* ST1 = new BT::SimpleTask(BT, [](BT::BehaviorTree* BT)
											   {
											   		...
											   		auto X = ER::Roll(0, Manager->Width);
											   		auto Y = ER::Roll(0, Manager->Height);
		...
		BT::SimpleTask* ST2 = new BT::SimpleTask(BT, [](BT::BehaviorTree* BT)
											 {
												...
												Enjon::Math::Vec3 Direction = EM::Vec3::Normalize(EM::Vec3(TargetCartesian, Target.z) - 
																									EM::Vec3(AI->CartesianPosition, AI->Position.z));

												AI->Velocity = Direction * 2.0f;
												// AI->VelocityGoal = Direction * 2.0f;

												if (Direction.x > 0) AI->ViewVector.x = 1;
												else AI->ViewVector.x = -1;
											 });
		...
		BT::ConditionalTask* CT = new BT::ConditionalTask(BT, [](BT::BehaviorTree* BT)
													   {
													   		...
													   		else
													   		{
																...
																// AI2->Velocity = Direction * speed;
																AI2->VelocityGoal = Direction * 2.0f;

																if (Direction.x > 0) AI2->ViewVector.x = 1;
																else AI2->ViewVector.x = -1;
