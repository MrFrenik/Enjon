...
{ 
	...
	{ 
		public: 
			...
			*/
			f32 Dot(const Vec2& other);

			
			/**
			* @brief Calculates square distance from other vector provided
			* @param other - vector used in distance calculation
			* @return f32 - squared distance between two vectors
			*/
			f32 DistanceTo(const Vec2& other);
			...
			*/	
			static Vec2 Normalize(const Vec2& vec) { return vec / vec.Length(); }
			...
			*/	
			static f32 Dot(Vec2& a, Vec2& b) { return a.Dot(b); }
